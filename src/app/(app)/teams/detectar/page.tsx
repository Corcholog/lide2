...
export default async function DetectTeamsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {detected.length === 0 ? (
        <p className="rounded-lg border border-dashed border-ink-700 px-6 py-12 text-center text-ink-300">
        ...
        <form action={createDetectedTeamsAction} className="flex flex-col gap-3">
          {detected.map((team, index) => (
            <div
              key={index}
              className="flex flex-col gap-3 rounded-lg border border-ink-800 bg-ink-900 p-4 sm:flex-row sm:items-center"
            >
              ...
              <label className="flex shrink-0 items-center gap-3">
                ...
                <input
                  ...
                  className="w-48 rounded border border-ink-700 bg-ink-950 px-3 py-1.5 text-sm outline-none focus:border-brand-aqua"
                />
              </label>
          ...
          <button
            type="submit"
            className="self-start rounded bg-brand-red px-5 py-2.5 font-medium text-white transition-colors hover:bg-brand-red-soft"
          >
