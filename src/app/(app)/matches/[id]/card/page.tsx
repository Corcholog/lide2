...
import { formatGold, formatNumber, riotId } from '@/lib/format'
...
export default async function MatchCardPage({ params }: PageProps<'/matches/[id]/card'>) {
  await requireUser()
  ...
  const highlight = (
    ...
    row
      ? {
          label,
          value: value(row),
          player: riotId(row.riot_game_name, row.riot_tag_line),
  ...
  const data: MatchCardData = {
    ...
    mvp: mvpRow
      ? {
          name: riotId(mvpRow.riot_game_name, mvpRow.riot_tag_line),
