...
describe('leer el formulario del plantel', () => {
  it('respeta el orden de la pantalla y distingue las filas nuevas', () => {
    const filas = readRosterForm(
      formulario([
        { clave: 'r1', nombre: 'Dario Ferro' },
        { clave: 'r2', nombre: 'Gregorio Aguilar' },
        { clave: 'nuevo-0', nuevo: true, nombre: 'Andrea Aranda' },
      ]),
    )
  ...
  it('lee la baja, que es un campo que sólo viaja cuando está tildado', () => {
    const filas = readRosterForm(
      formulario([
        { clave: 'r1', nombre: 'Dario Ferro', baja: true },
        { clave: 'r2', nombre: 'Gregorio Aguilar' },
      ]),
    )
  ...
  it('recorta los espacios: un nombre con espacios de más no es un nombre distinto', () => {
    const [fila] = readRosterForm(
      formulario([{ clave: 'r1', nombre: '  Dario Ferro  ', riot: ' DarioFerro#LAN ' }]),
    )

    expect(fila.nombre).toBe('Dario Ferro')
    expect(fila.riot).toBe('DarioFerro#LAN')
  })
})
    ...
    const plan = planRosterEdit(
      ...
      readRosterForm(
        formulario([
          ...
          { clave: 'r1', nombre: 'Dario Ferro', universidad: UNER, riot: 'DarioFerro#LAN' },
        ]),
      ),
      PLANTEL,
    )
