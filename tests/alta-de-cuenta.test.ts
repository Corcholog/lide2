...
describe('cargar un nick a mano', () => {
  ...
  it('da de alta la cuenta sin PUUID y la deja en el plantel', async () => {
    const result = await alta(equipo01, 'DarioFerro', 'LAN')
    ...
    expect(lineup.rows.map((row) => row.name)).toEqual(['DarioFerro'])
  })

  it('el mismo nick dos veces no crea dos cuentas', async () => {
    await alta(equipo01, 'DarioFerro', 'LAN')
    const repetida = await alta(equipo01, 'darioferro', 'lan')
  ...
  it('si la cuenta ya jugó la reusa en vez de duplicarla', async () => {
    ...
    const result = await alta(equipo01, 'DarioFerro')
  ...
  it('no muda a nadie de un equipo a otro por su cuenta', async () => {
    await alta(equipo01, 'DarioFerro', 'LAN')
    const mudanza = await alta(equipo15, 'DarioFerro', 'LAN')
